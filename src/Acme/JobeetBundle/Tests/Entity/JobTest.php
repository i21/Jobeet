...
class JobTest extends WebTestCase
{
	...
	public function setUp()
	{
		...
		// We have to close the connection after dropping the
		// database so we don't get "No databse selected" error
	...
	{
		$job = $this->em->createQuery('SELECT j FROM AcmeJobeetBundle:Job j')
			->setMaxResults(1)
			->getSingleResult();
    ...
    {
        $job = new Job();
        $value = new \DateTime(date('Y-m-d H:i:s', 86400 * 30));
        $job->setExpiresAt($value);
