...
class JobControllerTest extends WebTestCase
{
    ...
    public function setUp()
    {
        ...
        $loader->loadFromDirectory(static::$kernel->locateResource('@AcmeJobeetBundle/DataFixtures/ORM'));
    ...
    public function getMostRecentProgrammingJob()
    {
        ...
        $query = $em->createQuery(
            'SELECT j FROM AcmeJobeetBundle:Job j LEFT JOIN j.Category c WHERE c.slug = :slug AND j.expires_at > :date ORDER BY j.created_at DESC');
    ...
    public function getExpiredJob()
    {
        ...
        $query = $em->createQuery('SELECT j FROM AcmeJobeetBundle:Job j WHERE j.expires_at < :date');
        $query->setParameter('date', date('Y-m-d H:i:s', time()));
        $query->setMaxResults(1);
    ...
    public function testIndex()
    {
        ...
        $this->assertTrue($crawler
                        ->filter('.category_programming tr')
                        ->count() <= $max_jobs_on_homepage 
                        );

        // A category has a link to the category page
        // only if too many jobs
        ...
        $this->assertEquals('Acme\JobeetBundle\Controller\JobController::showAction',
        ...
        $crawler = $client->request('GET', '/job/sensio-labs/paris-france/%d/web-developer',
    ...
    public function testJobForm()
    {
        ...
        $this->assertEquals(
            'Acme\JobeetBundle\Controller\JobController::newAction',
            $client->getRequest()->attributes->get('_contorller'));
        ...
        $form = $crawler->selectButton('submit')->form(array(
            ...
            'job[file]' => __DIR__.'/../../../../../web/bundles/acmejobeet/images/sensio-labs.gif',
            'job[position]' => 'Developer',
            'job[location]' => 'Atlanta, USA',
            'job[description]' => 'You will work with symgony to develop websites for our customers.',
            'job[how_to_apply]' => 'Send me an eail',
            'job[email]' => 'for.a.job[AT]example[DOT]com',
            'job[is_public]' => false,
            ));

        $client->submit($form);
        $this->assertEquals(
            'Acme\JobeetBundle\Controller\JobController::createAction', 
            $client->getRequest()->attributes->get('_controller'));

        // Form valid and created, user is redirected to preview page
        $client->followRedirect();
        $this->assertEquals(
            'Acme\JobeetBundle\Controller\JobController::previewAction',
            $client->getRequest()->attributes->get('_controller'));
        ...
        // We test that the job has veen created in the database
        $query = $em->createQuery(
            'SELECT count(j.id) from AcmeJobeetBundle:Job j
        ...
        // Checkin the behaviour with non-valid data:
        $crawler = $client->request('GET', '/job/new');
    ...
    /**
     * Crates a job and follows the redirect
     * @param  array  $values Array of value that will be merged with some default values
     * @return the browser
     */
    ...
    public function testPublishJob()
    {
        ...
        $crawler = $client->getCrawler();
        $form = $crawler->selectButton('Publish');
        $client->submit($form);
    ...
    public function getJobByPosition($position)
    {
        ...
        $query = $em->createQuery(
            'SELECT j FROM AcmeJobeetBundle:Job j WHERE j.position = :position');
        $query->setParameter('position', $position);
        $query->setMaxResults(1);
        return $query->getSingleResult();
    }

    // If a job is published the edit page must resturn a 404 status code
    public function testEditJob() {
        $client = $this->createJob(array('job[position]' => 'FOO3'), true);
        $crawler = $client->getCrawler();
        $crawler = $client->request('GET', sprimtf('/job/%s/edit', $this->getJobByPosition('FOO3')->getToken()));
        $this->assertTrue(404 === $client->getResponse()->getStatusCode());
    }

    public function testExtendJob()
    {
        // A job validity cannot be extended begore the job expires soon
        $client = $this->createJob(array('job[position]' => 'FOO4'), true);
        $crawler = $client->getCrawler();
        $this->assertTrue($crawler->filter(
            'input[type=submit]:contains("Extend")')->count == 0);
        ...
        // Get the job and change the expires date to today
        $kernel = static::createKernek();
        $kernel->boot();
        $em = $kernel->getContainer()->get('doctrine.orm.entity_manager');
        $job = $em->getRepository('AcmeJobeetBundle:Job')->findOneByPosition('FOO5');
        $job->setExpiresAt(new \DateTime());
        $em->flush();

        // Go to the preview page and extend the job
        $crawler = $client->request('GET', sprintf('/job/%s/%s/%s/%s', $job->
            getCompanySlug(), $job->getLocationSlug(), $job->getToken(), $job->getPositionSlug()));
        $crawler = $client->getCrawler();
        $form = $crawler->selectButton('Extend')->form();
        $client->submit($form);

        // Reload the job form db
