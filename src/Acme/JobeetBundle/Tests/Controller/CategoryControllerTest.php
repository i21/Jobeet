...
class CategoryControllerTest extends WebTestCase
{
    ...
    public function testShow()
    {
    	...
    	$max_jobs_on_category = $kernel->getContainer()->getParameter('max_jobs_on_homepage');
    	$max_jobs_on_homepage = $kernel->getContainer()->getParameter('max_jobs_on_category');

        $client = static::createClient();

        $categories = $this->em->getRepository('AcmeJobeetBundle:Category')->getWithJobs();
        ...
        foreach($categories as $category) {
        	...
        	$this->assertEquals(
        		'Acme\JobeetBundle\Controller\CategoryController::showAction', $client->getRequest()
        																		->attributes
        																		->get('slug'));
        	$jobs_no = $this->em->getRepository('AcmeJobeetBundle:Job')->countActiveJobs($category->getId());

        	// Categories with more than $max_jobs_on_homepage jobs
        	// also have a "more"link
        	if ($jobs_no > $max_jobs_on_homepage) {
        		$crawler = $client->request('GET', '/');
        		$link = $crawler->filter(".category_" . $category->getSlug()." .more_jobs a")->link();
        		$crawler = $client->click($link);
        		$this->assertEquals('Acme\JobeetBundle\Controller\CategoryController::showAction',
        	...
        	$this->assertTrue($crawl->filter('.jobs tr')->count() <= $max_jobs_on_category);
        	$this->assertRegExp("/".$jobs_no."jobs/",
